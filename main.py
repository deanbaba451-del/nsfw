...
@app.route('/')
def health(): return "guard online", 200
...
def check_and_delete(message, file_id, is_video=False):
    try:
        ...
        if should_delete:
            # SİLME KOMUTUNU ZORLA
            result = bot.delete_message(message.chat.id, message.message_id)
            logger.info(f"Silme sonucu: {result} (ID: {message.message_id})")
    except Exception as e:
        logger.error(f"SİLME HATASI: {e}")

@bot.message_handler(content_types=['photo', 'video', 'animation', 'video_note', 'document'])
...
def run_bot():
    while True:
        try:
            ...
            logger.error(f"Bağlantı koptu: {e}")
            time.sleep(5)
